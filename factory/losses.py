...
def binary_lovasz(per_image=True):
    def func(input, target):

        loss = lovasz_hinge(input, target, per_image=per_image)
        return loss

    return func

def binary_lovasz2(per_image=True):
    def func(input, target):

        loss = 0.
        for c in range(4):
            input_c = input[:,c].contiguous()
            target_c = target[:,c].contiguous()
            loss += lovasz_hinge(input_c, target_c, per_image=per_image)
...
def bce_dice():
    def func(input, target):
        ...
        loss = bce_loss(input, target) + dice_loss(input, target)
        return loss
