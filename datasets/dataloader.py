...
from .datasets import SteelDataset_Seg, SteelDataset_Cls
from .stratified_sampler import StratifiedSampler
from .balance_sampler import FiveBalanceClassSampler


def get_dataloader(config, split, transform=None):
    dset = SteelDataset_Seg if config.TASK == 'seg' else SteelDataset_Cls
    dataset = dset(config, split, transform=transform)
    ...
    if is_train:
        ...
        elif config.SAMPLER == 'balance':
            sampler = FiveBalanceClassSampler(dataset)
        else:
            raise Exception('choose between stratified and balance.')
        # sampler = StratifiedSampler(dataset.labels)
        # sampler = StratifiedSampler(dataset.labels) if config.TASK =='seg' else FiveBalanceClassSampler(dataset)
        dataloader = FixedSizeDataLoader(dataset, steps=steps, batch_size=batch_size, num_workers=config.TRAIN.NUM_WORKERS,
